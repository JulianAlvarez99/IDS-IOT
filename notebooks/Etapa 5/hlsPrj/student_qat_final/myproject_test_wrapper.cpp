...
int main(int argc, char **argv) {
    ...
    if (fin.is_open() && fpr.is_open()) {
      ...
      nnet::copy_data_axi<float, input_axi_t, 0, 18>(in, inputs);
      output_axi_t outputs[N_OUT];
            ...
            if (e % CHECKPOINT == 0) {
                ...
                nnet::print_result<output_axi_t, 1>(outputs, std::cout, true);
            }
            e++;

            // hls-fpga-machine-learning insert tb-output
            nnet::print_result<output_axi_t, 1>(outputs, fout);
        }
        fin.close();
        fpr.close();
    } else {
